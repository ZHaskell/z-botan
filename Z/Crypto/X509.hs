...
import           Z.Data.CBytes     (CBytes, CString, withCBytesUnsafe)
...
readX509CertStart :: X509Cert -> IO T.Text
readX509CertStart (X509Cert cert) =
    withBotanStruct cert $ \ cert' ->
    T.Text <$> allocBotanBufferUnsafe 16
        (botan_x509_cert_get_time_starts cert')
...
readX509CertExpire :: X509Cert -> IO T.Text
readX509CertExpire (X509Cert cert) =
    withBotanStruct cert $ \ cert' ->
    T.Text <$> allocBotanBufferUnsafe 16
        (botan_x509_cert_get_time_expires cert')

-- | Return the time the certificate becomes valid, as seconds since epoch.
readX509CertNotBefore :: X509Cert -> IO Word64
readX509CertNotBefore (X509Cert cert) = do
  withBotanStruct cert $ \ cert' -> do
    (a, _) <- allocPrimUnsafe @Word64 $ \ time ->
...
readX509CertFingerPrint (X509Cert cert) ht =
    withBotanStruct cert $ \ cert' ->
    withCBytesUnsafe (hashTypeToCBytes ht) $ \ ht' ->
...
readX509CertSerial (X509Cert cert) =
    withBotanStruct cert $ \ cert' ->
    allocBotanBufferUnsafe V.smallChunkSize $
...
readX509CertIDAuthority (X509Cert cert) =
    withBotanStruct cert $ \ cert' ->
    allocBotanBufferUnsafe V.smallChunkSize $
...
readX509CertIDSubject (X509Cert cert) =
    withBotanStruct cert $ \ cert' ->
    allocBotanBufferUnsafe V.smallChunkSize $
...
readX509CertPubBits (X509Cert cert) =
    withBotanStruct cert $ \ cert' ->
    allocBotanBufferUnsafe V.smallChunkSize $
...
readX509CertDNIssuer ::
  ...
  Int ->
  IO V.Bytes
readX509CertDNIssuer (X509Cert cert) key ix =
    withBotanStruct cert $ \ cert' ->
...
readX509CertDNSubject ::
  ...
  Int ->
  IO V.Bytes
readX509CertDNSubject (X509Cert cert) key ix =
    withBotanStruct cert $ \ cert' ->
...
x509CertToBytes :: X509Cert -> IO V.Bytes
x509CertToBytes (X509Cert cert) =
    withBotanStruct cert $ \ cert' ->
    allocBotanBufferUnsafe V.smallChunkSize $
        botan_x509_cert_to_string cert'
...
verifyX509Cert ::
  ...
  Word64 ->
  IO Bool
verifyX509Cert (X509Cert cert) intermediates trusted path strength hostname refTime =
    withBotanStruct cert $ \ cert' ->
    withPrimVectorUnsafe intermediates $ \ intermediates' intermediatesOff intermediatesLen ->
    withPrimVectorUnsafe trusted $ \ trusted' trustedOff trustedLen ->
    withCBytesUnsafe path $ \ path' ->
    withCBytesUnsafe hostname $ \ hostname' -> do
        (a, _) <- allocPrimUnsafe @CInt $ \ ret ->
              throwBotanIfMinus_ $
                hs_botan_x509_cert_verify ret cert' intermediates' intermediatesOff
                    intermediatesLen trusted' trustedOff trustedLen path' strength hostname' refTime
        return $! a == 0

-- | Certificate path validation supporting Certificate Revocation Lists.
--
-- Verify a certificate. Returns 'True' if validation was successful, 'False' if unsuccessful.
--
verifyX509CertCRL ::
  -- | The certificate to be verified.
  X509Cert ->
  ...
  Word64 ->
  IO Bool
verifyX509CertCRL (X509Cert cert) intermediates trusted crls path strength hostname refTime =
    withBotanStruct cert $ \ cert' ->
    withPrimVectorUnsafe intermediates $ \ intermediates' intermediatesOff intermediatesLen ->
    withPrimVectorUnsafe trusted $ \ trusted' trustedOff trustedLen ->
    ...
    withCBytesUnsafe hostname $ \ hostname' -> do
        (a, _) <- allocPrimUnsafe @CInt $ \ ret ->
            throwBotanIfMinus_ $
                hs_botan_x509_cert_verify_with_crl ret cert' intermediates'
                    intermediatesOff intermediatesLen trusted' trustedOff trustedLen
                        crls' crlsOff crlsLen path' strength hostname' refTime
        return $! a == 0
