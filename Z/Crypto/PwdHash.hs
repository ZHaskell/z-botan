...
genBcrypt pwd rng n = do
    withPasswordUnsafe pwd $ \ pwd_p ->
        withRNG rng $ \ rng_p -> do
            r <- allocBotanBufferUnsafe 64 $ \ out out_len ->
                botan_bcrypt_generate out out_len pwd_p rng_p n 0
...
validBcrypt pwd hash = do
    withPasswordUnsafe pwd $ \ pwd_p ->
        withPrimVectorUnsafe hash $ \ hash_p hash_off hash_l -> do
            ret <- hs_botan_bcrypt_is_valid  pwd_p hash_p hash_off hash_l
